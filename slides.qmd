...
## [Homicide in the US]{.r-fit-text} {background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.15"}

```{r homicide-in-us, cache = TRUE}
load("./data/shr_rates_year.RData")
shr_rates_year |>
  filter(year >= 1987) |>
...
## [Homicide in Chicago]{.r-fit-text} {background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.15"}
...
## [Gun homicide is increasingly by adults]{.r-fit-text}{background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.15"}


```{r handgun-homicide}
load("./data/shr_props_year.RData")
col_vec <- setNames(c("#00BFC4", "#F8766D", "grey50"), c("Adolescence", "Early Adulthood", ""))
shr_props_year <- shr_props_year |>
  filter(between(age, 10, 60)) |>
  mutate(stage = case_when(
    between(age, 10, 20) ~ "Adolescence",
    between(age, 21, 45) ~ "Early Adulthood",
    TRUE ~ ""
  ))
...
## [Age and period effects on gun carrying]{.r-fit-text}{background-image="img/stephan-cassara-KnAIsBuitGg-unsplash.jpg" background-opacity="0.15"}

![](img/all-curves-weighted.png)
...
![](img/exposure_prediction_plot.png)

## Questions {background-image="img/clay-banks-nsAn3nSW5T0-unsplash.jpg" background-opacity="0.4"}
...
digraph G{
 ...
 sc1 [pos = "0,  1!", label = "Current\nContext"]
  si [pos = "0.5,  0!", label = "Situation"]
  sa [pos = "2.5,  0!", label = "Action"]
  sc2 [pos = "3,  1!", label = "Future\nContext"]
  i1 [pos = "-2, 0!", label = "Life\nHistory"]
  i2 [pos = "5, 0!", label = "New\nHistory"]
...
digraph G{
 ...
 sc1 [pos = "0,  3.5!", label = "Diffuse\nthreats"]
  si [pos = "0.5,  2.5!", label = "Generalized fear\nCynicism"]
  sa [pos = "2.5,  2.5!", label = "Adult gun\ncarrying"]
  sc2 [pos = "3,  3.5!", label = "Increased\nadult\nviolence"]
  i1 [pos = "-2, 2.5!", label = "Race\nSex"]
  ...
  bsc1 [pos = "0,  1!", label = "Immediate\nthreats"]
  bsi [pos = "0.5,  0!", label = "Specific fear"]
  bsa [pos = "2.5,  0!", label = "Youth gun\ncarrying"]
  bsc2 [pos = "3,  1!", label = "Increased\nyouth\nviolence"]
  bi1 [pos = "-2, 0!", label = "Race\nSex"]
...
## {.nostretch background-image="img/clay-banks-nsAn3nSW5T0-unsplash.jpg" background-opacity="0.15"}
